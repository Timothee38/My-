...
void main () {
	...
	int fichNum;
  	...
  	int octetRej, octetFich1, octetFich2, octetFich3, octetFich4, octetFich5;
	...
	for(i = 0; i < 67; i++) {		//On fait en sorte que la chaine soit remplie de \0
		tramTemp[i] = '\0';
	}
	...
	if (fichier == NULL) {
      ...
      fichNum = traiteligne(tramTemp, fichierRej,&rejets,&totales, &octetRej); //On analyse la ligne
			...
			for(i = 0; i < 67; i++) {		//Même chose qu'avant
				tramTemp[i] = '\0';
			}
		}
    printf("Rapport: %f \n Fichier 1: %d o, Fichier 2", (rejets/totales), octetFich1);
	}
  //Fermeture de tous les fichier (IMPORTANT)
	fclose(fichier1);
	fclose(fichier2);
	fclose(fichier3);
	fclose(fichier4);
