...
void ecritLigne(char* trame,FILE*fichier){
	int i, pointCaract,j;
	pointCaract = 0;
	int taille = calcTaille(trame)-2;
	int nbLignes=taille/7;
	int dernLigne=taille%7;
	printf("Trame : %s 1\n",trame);
	printf("taille=%d ; nbligne= %d ; dernLigne = %d \n", taille, nbLignes, dernLigne);
